...
namespace login.Reservas
{
    public partial class frmFacturaReserva : Form
    {
        ...
        public frmFacturaReserva(int clienteID, int canchaID, string cliente, string tipoDocumento, string documento, string correo, string telefono, string direccion, string cancha, DateTime fechaReserva, TimeSpan horaInicioReserva, TimeSpan horaFinReserva, string horario, int cantidadHoras, decimal precioHora, string ciudad = "", UCNuevaReserva reservaOriginal = null, Control contenedorOriginal = null, bool incluyeArbitro = false, decimal precioArbitro = 5.00m)
        {
            InitializeComponent();
            ConfigurarDetalle();
            ...
            decimal subtotalServicio = incluyeArbitro ? precioArbitro : 0m;
            ...
            dgvDetalleFactura.Rows.Add(cancha, horario, cantidadHoras, precioHora, descuento, subtotalCancha);

            if (incluyeArbitro)
                dgvDetalleFactura.Rows.Add("Servicio de árbitro", horario, 1, precioArbitro, 0m, precioArbitro);
        ...
        {
            string fecha = fechaReserva.ToString("yyyyMMdd");
            DataTable dt = conSQL.retornaRegistros("SELECT TOP 1 PromocionID, Nombre, Descuento, UnidadDescuento FROM Promociones WHERE Estado = 1 AND CanchaID = " + canchaID + " AND CONVERT(date, '" + fecha + "', 112) BETWEEN FechaInicio AND FechaFin ORDER BY PromocionID DESC");
            ...
            cmbPromocion.SelectedIndex = dt.Rows.Count > 1 ? 1 : 0;
        ...
        private void cmbPromocion_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                decimal valorDescuento = fila["Descuento"] == DBNull.Value ? 0 : Convert.ToDecimal(fila["Descuento"]);
                string unidad = fila["UnidadDescuento"] == DBNull.Value ? "$" : fila["UnidadDescuento"].ToString().Trim();
                descuento = unidad == "%" ? subtotalCancha * valorDescuento / 100m : valorDescuento;
            ...
            if (dgvDetalleFactura.Rows.Count > 0)
            {
                ...
                dgvDetalleFactura.Rows[0].Cells[5].Value = subtotalCancha;
            }
        }

        private void cmbPromocion_SelectedIndexChanged_1(object sender, EventArgs e)
        {
        ...
        {
            conSQL.retornaRegistros("UPDATE Promociones SET Estado = 0 WHERE Estado = 1 AND FechaFin < CAST(GETDATE() AS date)");
        }
        ...
        {
            Control contenedor = contenedorOriginal ?? Parent;

            if (contenedor == null)
            {
                MessageBox.Show("No se encontró el panel contenedor.");
                return;
            }

            UCNuevaReserva reserva = reservaOriginal ?? contenedor.Tag as UCNuevaReserva;

            if (reserva == null)
            {
                MessageBox.Show("No se pudo recuperar la reserva.");
                return;
            }
        ...
        private void btnFacturar_Click(object sender, EventArgs e)
        {
            ...
            int facturaID = conSQL.guardarReservaFactura(clienteID, canchaID, fechaReserva, horaInicioReserva, horaFinReserva, numeroFactura, dtpFechaEmision.Value, metodoPago, promocionIDSeleccionada, subtotal, descuento, iva, total, cancha, horario, cantidadHoras, precioHora, incluyeArbitro, precioArbitro);
            ...
            MessageBox.Show("Reserva y factura guardadas correctamente.", "Éxito", MessageBoxButtons.OK, MessageBoxIcon.Information);
